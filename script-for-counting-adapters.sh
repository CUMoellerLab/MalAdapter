...
input_directory="$1"
table_file="${input_directory}_occurrences_table.txt"
...
find "$input_directory" -type f -name "*.fasta" -o -name "*.fa" -o -name "*.fna" | while read -r fasta_file; do
    output_file="${fasta_file%.fasta}_processed_1A95AD3827A7.fasta"
    ...
    occurrences=$(grep -o "AGATCGGAAGAG" "$output_file" | wc -l)
    
    # Calculate distances between occurrences and the next '>' or 'NNN'
    distances_end=$(awk -v RS='AGATCGGAAGAG' -v FS='>' 'NR > 1 {print length($1)}' "$output_file")
    distances_ambig=$(awk -v RS='AGATCGGAAGAG' -v FS='NNN' 'NR > 1 {print length($1)}' "$output_file")
